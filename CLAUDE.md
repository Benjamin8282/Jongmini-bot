...
| AI | `/자캐커미션` | 등록 캐릭터를 AI로 화풍 변환 (원본 자세 유지) | `character_commission.py` |
| AI | `/자캐장면커미션` | 등록 캐릭터를 원하는 장면으로 AI 연출 (반실사) | `character_commission.py` |
...
- **장면연출**(`/자캐장면커미션`): Claude Sonnet 4 묘사 + 사용자 장면 → Stable Image Ultra text2image. 자세/배경 자유, 화풍 반실사(painterly) 고정.
- **화풍변환**(`/자캐커미션`): 원본 렌더를 알파 바운딩박스로 타이트 크롭 → Ultra img2img. 자세 원본 고정, 화풍 선택(반실사/애니 s0.45 / 수채화 s0.6+묘사 앵커).
- 모드별 입력만 노출하려고 커맨드를 2개로 분리(discord는 옵션 동적 숨김 불가).
...
- **타임라인 코드**: API 요청 시 `TIMELINE_CODES`로 필터링. 서약 코드(550~557), 강화/증폭(401,402), 봉인된 자물쇠(501) 등

### 타임라인 코드 매핑

| 코드 | 설명 | 알림 파이프라인 |
|------|------|----------------|
| 401 | 아이템 강화 | 강화/증폭 (before>=10만 알림) |
| 402 | 아이템 증폭 | 강화/증폭 (before>=10만 알림) |
| 501 | 봉인된 자물쇠 아이템 획득 | 봉인된 자물쇠 |
| 504 | 아이템 획득 | 장비 아이템 |
| 505 | 아이템 획득 | 장비 아이템 |
| 507 | 아이템 획득 | 장비 아이템 |
| 508 | 아이템 획득 | 장비 아이템 |
