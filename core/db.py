...
async def get_conn() -> aiosqlite.Connection:
    """영구 DB 연결 반환. 최초 호출 시 연결 생성 + PRAGMA 설정."""
    global _conn
    if _conn is None or not _conn._running:
...
async def init_db():
    ...
    try:
        ...
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_aph_item_created
            ON auction_price_history(item_id, created_at)
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_uas_item_enabled
            ON user_alert_settings(item_id, enabled)
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_uas_user_enabled
            ON user_alert_settings(user_id, enabled)
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_drc_mode_score
            ON dundam_ranking_cache(mode, score DESC)
        """)
        await conn.execute("""
