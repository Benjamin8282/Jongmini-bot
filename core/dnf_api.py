...
TIMELINE_CODES = "401,402,501,505,504,507,508,511,513,550,551,552,553,554,555,556,557"
...
async def fetch_mist_assimilation(server_id: str, character_id: str) -> dict | None:
    """안개서약 조회. {'level': int, 'expRate': str, 'status': [...]} 반환"""
    ...
    try:
        ...
        async with session.get(url, params=params) as response:
            if response.status == 200:
                ...
                logger.warning(f"안개서약 조회 실패: HTTP {response.status} - {character_id}")
    except Exception as e:
        logger.error(f"안개서약 조회 예외: {e}")
    return None
